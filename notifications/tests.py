...
class NotificationSignalTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.sound = Catch.objects.create(
            owner=self.user2,
            title="testcatch",
            latitude=0.0,
            longitude=0.0,
    ...
    def test_notification_created_for_new_catch(self):
        Follower.objects.create(owner=self.user1, followed=self.user2)
        ...
        self.assertEqual(notification.category, "new_catch")
        self.assertEqual(notification.title, "You have a new catch!")
